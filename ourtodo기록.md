...
- 쿠키 값으로 객체로 토큰, 아이디, 식별자 값을 주고 다시 parse하는 과정에서 문제 발생
- 해결이 안되서 결국 그냥 객체를 stringify하고 parse함.
- 자동로그인 구현
- 앞으로 로그아웃하면 쿠키 날리기.
- 로그인쿠키가 안맞으면 db 날리기
- 자동로그인 구현 완료
...
- set todo 작업 완료.
- 근데 그 인풋 id가 AI라서 계속 증가하는데 삭제하면 나머지 숫자가 정렬되면 좋겟음.

해야할거

- 자동로그인 로직 모델로 옮기기. -> 세션 저장까지 해야함
- db 털어서 화면에 띄우는거랑 삭제 기능
- 세션 db 무분별하게 많이 생김 -> 내가 로그인했을때 세션에 내 아이디 세션이 있으면 다 지우셈
...
- 기존 로그인 세션을 두시간으로 하고 요청있을때마다 저 라우터 거쳐서 2시간씩 다시 갱신 -> ok
- 자동로그인 로직 모델로 옮기기. -> 세션 저장까지 해야함 -> ok
- db 털어서 화면에 띄우는거랑 삭제 기능

## 21년 2월 15일

### 날씨 정확도 개선 및 날씨 아이콘 조정

- 날씨 정확도 개선을 위해 수치 100 200 과 같은 수치가 아닌, 아이콘을 불러와서 내가 만든 아이콘을 냅다 넣음
